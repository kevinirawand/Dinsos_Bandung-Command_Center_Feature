...
const utilsFunction = {
         ...
         $.ajax({
            ...
            success: function (data) {
               ...
               $(".cctv-list-anchor").each((index, element) => {
                  $(element).on('click', () => {
                     showCctvStream($(element).attr("url-data"), video);
   ...
   modalContentLoop(item) {
      content += `
      <div class="card text-center" style="width: 35rem; background-color: #e2e3e5;">
            <div class="d-flex justify-content-center mt-4" >
               <img class="card-img-top cctv-list-anchor" style="width: 100px;margin-top:20px" src="../../assets/images/video-play.png" url-data="${item.stream}" alt="Card image cap" style="max-width: 100px;">
            </div>
...
      </div>`
   ...
   tableViewContentLoop(item) {
      ...
      <div class="thumbnail">
         <div style="width: 100%; height: 250px; overflow: hidden;">
            <video class="table-video" src="" width="100%" stream-url="${item.stream}" controls></video>   
         </div>
   ...
   getCctvList() {
      ...
      $.ajax({
         url: "http://localhost:1337/cctv/cctv-location/list",
         type: 'get',
         dataType: 'JSON',
         async: false,
         success: function (data) {
   ...
   previewCctvTable() {
      ...
      videos.forEach((video) => {
         showCctvStream($(video).attr('stream-url'), video);
      })
   }
}
...
const showCctvStream = (url, element) => {
   $.ajax({
      type: "GET",
      headers: {
